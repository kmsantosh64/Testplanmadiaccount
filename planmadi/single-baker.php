...
<form id="partnerdetailcapture" method="post" action="<?php echo home_url(); ?>/baker-enquiry/">
                        ...
                        <input type="submit" class="cta-button" value="Order Your Cake Here"/>
                        </form>
...
$addonlists = get_field('addon_list');

if ($addonlists) {
    $itemsToPass = array(); 
    foreach ($addonlists as $addonlist) {
        $serviceitems = $addonlist['addon_names'];
        
        if (is_string($serviceitems)) {
            
            $serviceitems = explode(',', $serviceitems);
        }
    ...
    $_SESSION['itemsToPass'] = $itemsToPass;
...
$caketypes = get_field('caketypelist');

if ($caketypes) {
    $cakeItemsToPass = array(); 
    foreach ($caketypes as $caketype) {
        $cakeservice = $caketype['caketypenames'];
        
        if (is_string($cakeservice)) {
            $cakeservice = explode(',', $cakeservice);
        }

        if (is_array($cakeservice)) {
            foreach ($cakeservice as $cakeitems) {
                $cakeitems = trim($cakeitems); 
                if (!empty($cakeitems)) {
                    $cakeItemsToPass[] = $cakeitems; 
                }
            }
        }
    }
    ...
    $_SESSION['cakeItemsToPass'] = $cakeItemsToPass;
}
...
$creamtypes = get_field('creampreference');

if ($creamtypes) {
    $creamItemsToPass = array(); 
    foreach ($creamtypes as $creamtype) {
        $creamservice = $creamtype['creamlist'];
        
        if (is_string($creamservice)) {
            $creamservice = explode(',', $creamservice);
        }

        if (is_array($creamservice)) {
            foreach ($creamservice as $creamitems) {
                $creamitems = trim($creamitems); 
                if (!empty($creamitems)) {
                    $creamItemsToPass[] = $creamitems; 
                }
            }
        }
    }
    ...
    $_SESSION['creamItemsToPass'] = $creamItemsToPass;
}
